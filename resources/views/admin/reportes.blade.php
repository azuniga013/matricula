        ...
        catalogo: {
            academicos: [
                ...
                { id: 'por-docente', label: 'Matriculados por Docente', url: '/api/v1/reportes/academicos/por-docente', fechaDesde: false, fechaHasta: false, periodo: true, sucursal: true, estadoMatricula: true,
                  ...
                  totalesFn: (filas) => [{ label: 'Total matriculados', valor: filas.reduce((a, f) => a + Number(f.total_matriculados || 0), 0) }] },
                { id: 'grupo', label: 'Alumnos por Horario', url: '/api/v1/reportes/academicos/grupo', fechaDesde: false, fechaHasta: false, oferta: true, periodo: true, sucursal: true,
                ...
                { id: 'calificaciones-grupo', label: 'Calificaciones por Horario', url: '/api/v1/reportes/academicos/calificaciones-por-grupo', fechaDesde: false, fechaHasta: false, oferta: true, periodo: true, sucursal: true,
            ...
            financieros: [
                ...
                { id: 'pagos-pendientes-por-estudiante', label: 'Pagos Pendientes por Estudiante', url: '/api/v1/reportes/financieros/pagos-pendientes-por-estudiante', fechaDesde: false, fechaHasta: false, sucursal: true, periodo: true,
                  columns: [ {key:'estudiante_codigo', label:'Código', mono:true}, {key:'nombre_completo', label:'Estudiante', fn: f => `${f.nombre || ''} ${f.apellido || ''}`}, {key:'correo', label:'Correo'}, {key:'telefono', label:'Teléfono'}, {key:'sucursal_codigo', label:'Sucursal', mono:true, fn: f => `${f.sucursal_codigo || ''} · ${f.sucursal_nombre || ''}`}, {key:'periodo_nombre', label:'Período'}, {key:'nivel_nombre', label:'Nivel'}, {key:'horario_nombre', label:'Horario'}, {key:'docente_nombre', label:'Docente', fn: f => `${f.docente_codigo || ''} · ${f.docente_nombre || ''}`}, {key:'pagos_pendientes', label:'Pagos', numeric:true}, {key:'monto_pendiente_total', label:'Saldo', numeric:true, money:true} ],
                  totalesFn: (filas) => [{ label: 'Estudiantes', valor: filas.length }, { label: 'Saldo total', valor: M(filas.reduce((a, f) => a + Number(f.monto_pendiente_total || 0), 0)) }] },
                { id: 'pendientes-por-estudiante', label: 'Cuentas Pendientes por Estudiante', url: '/api/v1/reportes/financieros/pendientes-por-estudiante', fechaDesde: false, fechaHasta: false, estudiante: true, single: true,
                  columns: [ {key:'estudiante_codigo', label:'Código', mono:true}, {key:'nombre_completo', label:'Estudiante', fn: f => `${f.nombre || ''} ${f.apellido || ''}`}, {key:'correo', label:'Correo'}, {key:'telefono', label:'Teléfono'}, {key:'sucursal_codigo', label:'Sucursal', mono:true, fn: f => `${f.sucursal_codigo || ''} · ${f.sucursal_nombre || ''}`}, {key:'periodo_nombre', label:'Período'}, {key:'nivel_nombre', label:'Nivel'}, {key:'horario_nombre', label:'Horario'}, {key:'docente_nombre', label:'Docente', fn: f => `${f.docente_codigo || ''} · ${f.docente_nombre || ''}`}, {key:'obligaciones_pendientes', label:'Obligaciones', numeric:true}, {key:'saldo_pendiente_total', label:'Saldo', numeric:true, money:true} ],
                  totalesFn: (filas) => [{ label: 'Estudiantes', valor: filas.length }, { label: 'Saldo total', valor: M(filas.reduce((a, f) => a + Number(f.saldo_pendiente_total || 0), 0)) }] },
