        ...
        catalogo: {
            ...
            recibos: [
                { id: 'por-orden', label: 'Recibos por Orden Numérico', url: '/api/v1/reportes/recibos/por-orden', fechaRequerida: true, sucursal: true, estadoRecibo: true, paginado: true,
                  columns: [ {key:'numero_recibo', label:'# Recibo', mono:true, fn: f => String(f.numero_recibo).padStart(6, '0')}, {key:'estudiante', label:'Estudiante', fn: f => `${f.estudiante?.nombre || ''} ${f.estudiante?.apellido || ''}`}, {key:'metodo_pago', label:'Método', fn: f => f.metodo_pago?.nombre || '-'}, {key:'monto_total', label:'Monto', numeric:true, money:true}, {key:'estado', label:'Estado', fn: f => BADGE(f.estado, f.estado === 'emitido' ? 'success' : f.estado === 'anulado' ? 'danger' : 'info')}, {key:'fecha_recibo', label:'Fecha', fn: f => F(f.fecha_recibo || f.creado_en)} ] },
                { id: 'por-concepto-detalle', label: 'Recibos por Concepto (Detalle)', url: '/api/v1/reportes/recibos/por-concepto-detalle', fechaRequerida: true, sucursal: true, paginado: true,
                  columns: [ {key:'numero_recibo', label:'# Recibo', mono:true, fn: f => String(f.numero_recibo).padStart(6, '0')}, {key:'estudiante', label:'Estudiante', fn: f => `${f.estudiante?.nombre || ''} ${f.estudiante?.apellido || ''}`}, {key:'concepto_pago', label:'Concepto', fn: f => f.concepto_pago?.nombre || '-'}, {key:'metodo_pago', label:'Método', fn: f => f.metodo_pago?.nombre || '-'}, {key:'monto_total', label:'Monto', numeric:true, money:true}, {key:'estado', label:'Estado', fn: f => BADGE(f.estado, f.estado === 'emitido' ? 'success' : f.estado === 'anulado' ? 'danger' : 'info')}, {key:'fecha_recibo', label:'Fecha', fn: f => F(f.fecha_recibo || f.creado_en)} ] },
