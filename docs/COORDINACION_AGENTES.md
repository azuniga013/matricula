...
- 2026-08-25: avance del frente `P-059`: se detectó que el flujo no estaba cerrado porque faltaba persistir la URL real del enlace anticipado. Se agregó la migración `2026_08_24_000006_add_enlace_url_to_enlaces_pago_table.php`, el modelo `EnlacePago` ya contempla `enlace_url`, y el modal admin en `admin/pagos` ahora captura explícitamente la URL real y muestra `estado_operativo`. También se avanzó en el ciclo `reservado -> usado/desuso`: `ResolverEnlacePagoDisponible` ya puede reservar con `asignado_a_pago_id`/`asignado_a_estudiante_id`, y los casos de uso `AprobarPago`/`RechazarPago` ya marcan enlaces `usado` o `desuso` según corresponda. Verificación añadida 2026-08-25: `PagoTest::test_registrar_pago_asigna_link_anticipado_disponible` y `PortalEstudianteTest::test_registrar_pago_link_automatico_usa_enlace_valido|test_registrar_pago_link_automatico_rechaza_enlace_invalido`. Si otro agente toca este frente, no reintroducir el supuesto anterior de usar `codigo` como si fuera la URL del enlace.
- 2026-08-25: avance del frente `P-052`: se subió la versión de la APK docente a `0.1.14` (`versionCode 15`) y la compilación `./gradlew assembleRelease` terminó en verde. La APK generada quedó en `mobile-docentes/android/app/build/outputs/apk/release/app-release.apk`. Si otro agente retoma el frente, no bajar el `versionCode` ni volver a `0.1.13`.
- 2026-08-25: frente `P-059` cerrado. Contrato final: el enlace anticipado se captura con `enlace_url` real y `metodo_pago_id`; la reserva automática filtra por método, monto y `estado_operativo=disponible`; al reservar se amarra a pago/estudiante, pasa a `usado` al confirmarse/aprobarse y a `desuso` al reengancharse o rechazarse. No reabrir el esquema viejo de usar `codigo` como si fuera la URL del enlace ni quitar el `estado_operativo` de la UI admin.
...
- **Actualización documental obligatoria.** Si intervienen varios agentes en un frente, cada avance relevante, cambio de criterio, prueba agregada o ajuste de alcance debe reflejarse en `docs/PENDIENTES.md` y, cuando aplique, en este mismo documento antes de soltar el área. No dejar cambios coordinados solo en código.
