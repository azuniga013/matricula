...
| `P-021` | `en_progreso` | Crear APK offline para docentes. | La guía de inicio a fin está en `docs/APK_DOCENTES_OFFLINE.md`: APK interna, usuarios administrativos docentes, SQLite, cola de sincronización, conflictos, seguridad, API y firma. Se reforzó el backend para que un docente no pueda acceder a calificaciones de ofertas ajenas. Pendiente construir, probar y firmar la aplicación. |
