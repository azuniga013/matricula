...
| `P-021` | `en_progreso` | Crear APK offline para docentes. | Base Expo en `mobile-docentes/`: login administrativo de docente, SecureStore, SQLite, ofertas, alumnos, asistencia, notas y cola offline. El panel administra publicaciones y la URL pública `/apk/docentes` entrega solo la APK firmada activa. Pendiente endpoints de sincronización, pruebas en dispositivo, firma y primera publicación. |
