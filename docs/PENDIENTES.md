...
| `P-021` | `en_progreso` | Crear APK offline para docentes. | Base Expo en `mobile-docentes/`: login administrativo de docente, SecureStore, SQLite, ofertas, alumnos, asistencia, notas y cola offline. Se compiló localmente `app-release.apk` (61 MB), pero usa certificado Android Debug y no es publicable. El panel administra publicaciones y la URL pública `/apk/docentes` entrega solo la APK firmada activa. Pendiente llave institucional, prueba de login en dispositivo, endpoints de sincronización y primera publicación. |
