...
| `P-014` | `completado` | Mostrar cuentas bancarias de depósito en el Dashboard del Estudiante. | El Dashboard lista banco, nombre, número y tipo de cada cuenta activa, con acceso a registrar el pago. Verificado con `php artisan view:cache` y `npm run build`. |
