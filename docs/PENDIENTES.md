...
| `P-021` | `en_progreso` | Crear APK offline para docentes. | Base Expo en `mobile-docentes/`: login administrativo de docente, SecureStore, SQLite, ofertas, alumnos, asistencia, notas y cola offline. Dependencias instaladas y export Android validado. `docs/APK_DOCENTES_OFFLINE.md` es obligatorio y se actualiza en cada cambio. Pendiente endpoints de sincronización, pruebas en dispositivo y firma APK. |
